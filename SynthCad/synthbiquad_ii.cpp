...
CSynthBiQuadii::CSynthBiQuadii(int type ,CSynthParameters *Params, CSynthObject *pParent):CSynthObject(OBJECT_TYPE_BIQUAD,Params,pParent)
{
	//ctor
	m_Z1 = 0.0;
	m_Z2 = 0.0;
...
/** @brief (one liner)
  ...
  */
void CSynthBiQuadii::Run(void)
{
	float result,result1,sample;
	//---------------------
	// Update Filter Freq
	//---------------------
	if(m_pFreq)
	...
	sample = m_pIn->GetData();
     /* compute result */
	result1 = sample - m_a1 * m_Z1 - m_a2 * m_Z2;
	result = result1 * m_b0 + m_Z1 * m_b1 + m_Z2 * m_b2;
   m_Z2 = m_Z1;
   m_Z1 = result1;
...
/** @brief (one liner)
  ...
  * (documentation goes here)
  */
void CSynthBiQuadii::CalcCoefficients(int type,float freq, float Q)
{
